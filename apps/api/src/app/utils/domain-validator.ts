...
export class DomainValidator {
	static readonly EXPECTED_CNAME =
	...
	static async validateCnameConfiguration(
		domain: string,
	): Promise<Result<DomainValidationResult>> {
		...
		if (!domainValidation.success) {
			return {
				data: null,
				error:
					domainValidation.error.errors[0]?.message || "Invalid domain format",
			};
		}
			...
			if (cnameRecords && cnameRecords.length > 0) {
				...
				return {
					data: {
						...
						error: isValid
							? undefined
							: `CNAME points to ${cnameRecord}, expected ${expectedCname}`,
					},
					error: null,
				};
			}
