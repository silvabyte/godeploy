...
describe("DomainValidator", () => {
	describe("isValidDomainFormat", () => {
		it("should validate correct domain formats", () => {
			...
			expect(
				DomainValidator.isValidDomainFormat("sub-domain.example.com"),
			).toBe(true);
			...
			expect(DomainValidator.isValidDomainFormat("a.b.c.example.com")).toBe(
				true,
			); // Multiple subdomains
			expect(DomainValidator.isValidDomainFormat("my-app.example.co.uk")).toBe(
				true,
			); // Country TLDs
			expect(DomainValidator.isValidDomainFormat("app123.example.io")).toBe(
				true,
		...
		it("should reject invalid domain formats", () => {
			...
			expect(DomainValidator.isValidDomainFormat("http://example.com")).toBe(
				false,
			); // Protocol
			expect(DomainValidator.isValidDomainFormat("example.c")).toBe(false); // TLD too short
			expect(DomainValidator.isValidDomainFormat(`${"a".repeat(64)}.com`)).toBe(
				false,
			); // Label too long
		});
	});
	...
	describe("normalizeHostname", () => {
		it("should normalize hostnames correctly", () => {
			expect(DomainValidator.normalizeHostname("EXAMPLE.COM")).toBe(
				"example.com",
			);
			expect(DomainValidator.normalizeHostname("example.com.")).toBe(
				"example.com",
	...
	describe("validateCnameConfiguration", () => {
		it("should reject invalid domain format", async () => {
			const result =
				await DomainValidator.validateCnameConfiguration("invalid..domain");
		...
		it("should transform domain to lowercase", async () => {
			...
			const result = await DomainValidator.validateCnameConfiguration(
				"THISISANONEXISTENTDOMAINFORSURE123456.COM",
			);
		...
		it("should handle real domain with correct CNAME", async () => {
			...
			const result =
				await DomainValidator.validateCnameConfiguration("audetic.ai");

			expect(result.error).toBeNull();
			expect(result.data).toBeDefined();
			expect(result.data?.isValid).toBe(true);
			expect(result.data?.cnameRecord).toBe(
				"godeploy-nginx-o3dvb.ondigitalocean.app",
			);
		});
		...
		it("should handle domain without CNAME", async () => {
			...
			const result =
				await DomainValidator.validateCnameConfiguration("google.com");
	...
	describe("checkDomainAvailability", () => {
		it("should return false for domain with correct CNAME", async () => {
			...
			const result =
				await DomainValidator.checkDomainAvailability("audetic.ai");
		...
		it("should return true for non-existent domains", async () => {
			const result = await DomainValidator.checkDomainAvailability(
				"thisisanonexistentdomainforsure123456.com",
			);
	...
	describe("EXPECTED_CNAME configuration", () => {
		it("should have the expected default value or env value", () => {
			...
			const expectedValue =
				process.env.GODEPLOY_CNAME_TARGET ||
				"godeploy-nginx-o3dvb.ondigitalocean.app";
