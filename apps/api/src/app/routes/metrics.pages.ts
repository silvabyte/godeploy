...
function genSlug(base?: string) {
	...
	const fromTitle = base
		...
		.replace(/[^a-z0-9]+/g, "-")
		.replace(/(^-|-$)/g, "");
	return fromTitle && fromTitle.length >= 3
		? `${fromTitle}-${random}`
		: `metrics-${random}`;
}
...
export default async function (fastify: FastifyInstance) {
	...
	fastify.post("/api/metrics/pages", {
		schema: {
			...
			response: {
				201: zodToJsonSchema(pageResponse),
				400: zodToJsonSchema(z.object({ error: z.string() })),
			},
		},
		handler: async (
			request: FastifyRequest<{ Body: CreateMetricsPageBody }>,
			reply: FastifyReply,
		) => {
			...
			if (result.error || !result.data) {
				return reply
					.code(500)
					.send({ error: result.error || "Failed to create page" });
			}
	...
	fastify.patch("/api/metrics/pages/:id", {
		schema: {
			...
			response: {
				200: zodToJsonSchema(pageResponse),
		...
		handler: async (
			...
			if (byId.data.tenant_id !== tenant_id)
			...
			if (updated.error || !updated.data)
				return reply
					.code(500)
					.send({ error: updated.error || "Update failed" });
	...
	fastify.get("/api/metrics/pages", {
		schema: {
			security: [{ bearerAuth: [] }],
			response: { 200: zodToJsonSchema(listResponse) },
		},
