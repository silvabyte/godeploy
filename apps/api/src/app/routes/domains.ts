...
const checkDomainAvailabilitySchema = z.object({
	...
	projectId: z
		.string()
		.optional()
		.describe("Project ID to exclude from availability check (for updates)"),
});
...
const checkDomainAvailabilityJsonSchema = zodToJsonSchema(
	checkDomainAvailabilitySchema,
);
const domainValidationResponseJsonSchema = zodToJsonSchema(
	domainValidationResponseSchema,
);
const domainAvailabilityResponseJsonSchema = zodToJsonSchema(
	domainAvailabilityResponseSchema,
);
const cnameTargetResponseJsonSchema = zodToJsonSchema(
	cnameTargetResponseSchema,
);

// Type exports
type ValidateDomainBody = z.infer<typeof validateDomainSchema>;
type CheckDomainAvailabilityBody = z.infer<
...
export default async function (fastify: FastifyInstance) {
	...
	const ensureDomainRegistered = async (
		domain: string,
		...
		if (!addResult.ok) {
			request.log.error(
				{ domain, error: addResult.error },
				"Failed to register domain with DigitalOcean App Platform",
			);
		}
	};
	...
	fastify.post("/api/domains/check-availability", {
		...
		handler: async (
			...
			const isAvailable =
				availabilityResult.data && validationResult.data?.isValid === true;
