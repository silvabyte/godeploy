...
export class StorageService {
	...
	private static readonly DEFAULT_FILE_CONCURRENCY = Math.max(
		1,
		Number(
			process.env.UPLOAD_FILE_CONCURRENCY ||
				process.env.UPLOAD_CONCURRENCY ||
				"1",
		),
	);
	private static readonly DEFAULT_PART_CONCURRENCY = Math.max(
		1,
		Number(
			process.env.UPLOAD_PART_CONCURRENCY ||
				process.env.UPLOAD_CONCURRENCY ||
				"1",
		),
	);
		...
		try {
			...
			if (!spacesEndpoint || !bucketName) {
				return {
					data: null,
					error:
						"Storage not configured: DIGITAL_OCEAN_SPACES_ENDPOINT or DIGITAL_OCEAN_SPACES_BUCKET is missing",
				};
			}
			if (
				...
				return {
					data: null,
					error:
						"Storage not configured: DIGITAL_OCEAN_SPACES_KEY/SECRET is missing",
				};
			}
	...
	private async uploadDirectory(
		...
		async function* walk(
			currentDir: string,
		): AsyncGenerator<{ fullPath: string; key: string; name: string }> {
			const [openErr, dir] = await to(fs.opendir(currentDir));
			if (openErr || !dir) {
				throw new Error(
					`Failed to open directory: ${currentDir} - ${openErr?.message}`,
				);
			}
			try {
