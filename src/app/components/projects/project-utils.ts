 ...
 */
export function validateAndTransformProjectName(rawName: string): Result<ProjectNameValidation> {
  if (!rawName) {
    return {
  ...
  const name = StringFormatter.from(rawName).remove.trailing('-').remove.leading('-').toString()
...
export function addUrlToProject(projects: Project[]): (Project & { url: string })[]
