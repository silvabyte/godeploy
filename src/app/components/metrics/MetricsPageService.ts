...
export class MetricsPageService extends BaseService {
  ...
  async createPage(page: Omit<MetricsPage, 'id' | 'created_at' | 'updated_at'>): Promise<Result<MetricsPage>> {
    return this.create<MetricsPage>(page)
  }

  async updatePage(id: string, data: Partial<MetricsPage>): Promise<Result<MetricsPage>> {
