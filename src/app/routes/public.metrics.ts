...
export default async function (fastify: FastifyInstance) {
  ...
  fastify.get('/api/public/metrics/:slug', {
    schema: {
      ...
      response: { 200: zodToJsonSchema(pageResponseSchema), 404: zodToJsonSchema(z.object({ error: z.string() })) },
    },
    handler: async (request: FastifyRequest<{ Params: { slug: string } }>, reply: FastifyReply) => {
      ...
      if (pageResult.error || !pageResult.data || pageResult.data.is_public !== true) {
  ...
  fastify.get('/api/public/metrics/:slug/deploy-frequency', {
    ...
    handler: async (
      ...
      request.measure.start('public_metrics_frequency', { slug, from: fromStr, to: toStr })
      ...
      const deploysResult = await request.db.deploys.getSuccessfulDeploysInRange(projectIds, fromStr, toStr)
      if (deploysResult.error || !deploysResult.data) {
        request.measure.failure(deploysResult.error || 'Failed to fetch deploys')
      ...
      const series = projectIds.map((pid) => ({
        projectId: pid,
        data: zeroFillDailySeries(fromStr.slice(0, 10), toStr.slice(0, 10), countsByProject[pid] ?? {}),
      }))
