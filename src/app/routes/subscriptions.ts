...
export default async function (fastify: FastifyInstance) {
  ...
  fastify.get('/api/subscriptions/current', {
    ...
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const subscriptionResult = await request.db.subscriptions.getSubscription(tenant_id)
  ...
  fastify.post('/api/subscriptions', {
    ...
    handler: async (
      ...
      const subscriptionResult = await request.db.subscriptions.createSubscription(subscriptionData)

      if (subscriptionResult.error || !subscriptionResult.data) {
        request.measure.failure(subscriptionResult.error || 'Failed to create subscription')
        return reply.code(500).send({
  ...
  fastify.patch('/api/subscriptions/:id', {
    ...
    handler: async (
      ...
      const subscriptionResult = await request.db.subscriptions.updateSubscription(request.params.id, request.body)

      if (subscriptionResult.error || !subscriptionResult.data) {
        request.measure.failure(subscriptionResult.error || 'Failed to update subscription')
        return reply.code(500).send({
  ...
  fastify.post('/api/subscriptions/:id/cancel', {
    ...
    handler: async (
      ...
      const cancelResult = await request.db.subscriptions.cancelSubscription(request.params.id)
