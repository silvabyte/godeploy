...
import { addTokenToUrl, extractBearerToken, parseUrlHash } from '../components/auth/token-utils.js'
...
export default async function (fastify: FastifyInstance) {
  ...
  fastify.post('/api/auth/change-password', {
    ...
    handler: async (
      ...
      const result = await request.db.auth.changePassword(tokenResult.data!, currentPassword, newPassword)
  ...
  fastify.post('/api/auth/reset-password', {
    ...
    handler: async (
      ...
      const { email, redirect_uri } = request.body
      const result = await request.db.auth.requestPasswordReset(email, redirect_uri)
  ...
  fastify.post('/api/auth/reset-password/confirm', {
    ...
    handler: async (
      ...
      const result = await request.db.auth.confirmPasswordReset(token, newPassword)
